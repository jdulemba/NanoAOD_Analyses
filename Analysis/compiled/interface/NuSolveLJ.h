...
public:
  ...
  LorentzVector(const np::ndarray& arr) {
    double* npdata = (double*)(void*) arr.get_data();
    x_ = npdata[0]; 
    y_ = npdata[1]; 
    z_ = npdata[2]; 
    e_ = npdata[3];
...
void run_nu_solver(const np::ndarray& lep, const np::ndarray& jet, const np::ndarray& met, np::ndarray& out) {
  NuSolveLJ solver;
