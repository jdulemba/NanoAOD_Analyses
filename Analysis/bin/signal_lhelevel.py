...
class MyAnalyzer(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        outgoing = lhe_parts[lhe_parts.status == 1]
        tops = (outgoing[:, :3].sum(axis=1))
        tbars = (outgoing[:, 3:].sum(axis=1))
        #set_trace()
        ttbars = tops+tbars
            # calculate ctstar
        top_ctstar, tbar_ctstar = make_vars.ctstar(tops, tbars,flatten=True)
        ...
        for rewt_sys in self.reweight_systematics_to_run:
            ...
            if isInt_:
                pos_evts = evt_wt > 0
                output["mtt"].fill(dataset=f"{signame}_pos", sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                output["mtop"].fill(dataset=f"{signame}_pos", sys=rewt_sys, mtop=ak.flatten(tops.mass, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                output["pt_top"].fill(dataset=f"{signame}_pos", sys=rewt_sys, pt=ak.flatten(tops.pt, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                output["pt_tt"].fill(dataset=f"{signame}_pos", sys=rewt_sys, pt=ak.flatten(ttbars.pt, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                output["eta_top"].fill(dataset=f"{signame}_pos", sys=rewt_sys, eta=ak.flatten(tops.eta, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                output["eta_tt"].fill(dataset=f"{signame}_pos", sys=rewt_sys, eta=ak.flatten(ttbars.eta, axis=None)[pos_evts], weight=evt_wt[pos_evts])
                ...
                output["mtt_vs_top_ctstar_abs"].fill(dataset=f"{signame}_pos", sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None)[pos_evts], ctstar_abs=np.abs(top_ctstar)[pos_evts], weight=evt_wt[pos_evts])

                neg_evts = evt_wt < 0
                output["mtt"].fill(dataset=f"{signame}_neg", sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                output["mtop"].fill(dataset=f"{signame}_neg", sys=rewt_sys, mtop=ak.flatten(tops.mass, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                output["pt_top"].fill(dataset=f"{signame}_neg", sys=rewt_sys, pt=ak.flatten(tops.pt, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                output["pt_tt"].fill(dataset=f"{signame}_neg", sys=rewt_sys, pt=ak.flatten(ttbars.pt, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                output["eta_top"].fill(dataset=f"{signame}_neg", sys=rewt_sys, eta=ak.flatten(tops.eta, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                output["eta_tt"].fill(dataset=f"{signame}_neg", sys=rewt_sys, eta=ak.flatten(ttbars.eta, axis=None)[neg_evts], weight=evt_wt[neg_evts])
                ...
                output["mtt_vs_top_ctstar_abs"].fill(dataset=f"{signame}_neg", sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None)[neg_evts], ctstar_abs=np.abs(top_ctstar)[neg_evts], weight=evt_wt[neg_evts])
            else:
                output["mtt"].fill(dataset=signame, sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None), weight=evt_wt)
                output["mtop"].fill(dataset=signame, sys=rewt_sys, mtop=ak.flatten(tops.mass, axis=None), weight=evt_wt)
                output["pt_top"].fill(dataset=signame, sys=rewt_sys, pt=ak.flatten(tops.pt, axis=None), weight=evt_wt)
                output["pt_tt"].fill(dataset=signame, sys=rewt_sys, pt=ak.flatten(ttbars.pt, axis=None), weight=evt_wt)
                output["eta_top"].fill(dataset=signame, sys=rewt_sys, eta=ak.flatten(tops.eta, axis=None), weight=evt_wt)
                output["eta_tt"].fill(dataset=signame, sys=rewt_sys, eta=ak.flatten(ttbars.eta, axis=None), weight=evt_wt)
                ...
                output["mtt_vs_top_ctstar_abs"].fill(dataset=signame, sys=rewt_sys, mtt=ak.flatten(ttbars.mass, axis=None), ctstar_abs=np.abs(top_ctstar), weight=evt_wt)
