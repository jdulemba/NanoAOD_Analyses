...
nnlo_corr = load(os.path.join(proj_dir, "Corrections", base_jobid, cfg_pars["corrections"]["nnlo"]["filename"]))[cfg_pars["corrections"]["nnlo"]["var"]]
...
class Analyzer(processor.ProcessorABC):
    def __init__(self):
        ...
        self.nnlo_corr = {"Var" : cfg_pars["corrections"]["nnlo"]["var"], "Correction" : nnlo_corr}
    ...
    def process(self, events):
        ...
        weights.add("NLOewYt1", np.copy(ewk_wts_dict["Rebinned_KFactor_1.0"]))
            ## NNLO QCD weights
        weights.add("NNLOqcd", np.copy(MCWeights.get_nnlo_weights(self.nnlo_corr, events)))
        ...
        for sys in ["nosys", "NLOewYt1", "NNLOqcd", "NNLOqcd_NLOewYt1"]:
            if sys == "nosys":
                evt_wt = np.copy(weights.partial_weight(["genweight"]))
            elif sys == "NNLOqcd_NLOewYt1":
                evt_wt = np.copy(weights.weight())
            else:
                evt_wt = np.copy(weights.partial_weight(["genweight", sys]))
