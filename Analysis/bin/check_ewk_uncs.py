...
ewk_Otto = load(os.path.join(proj_dir, "Corrections", base_jobid, cfg_pars["corrections"]["ewk_uncs"]))
...
class Analyzer(processor.ProcessorABC):
    def __init__(self):
        ...
        self.ewk_Otto = ewk_Otto
        self.ewk_Afiq = ewk_Afiq
    ...
    def process(self, events):
        ...
        Otto_ewk_wts_dict = MCWeights.get_Otto_ewk_weights(self.ewk_Otto, events)
        Afiq_ewk_wts_dict = MCWeights.get_ewk_weights(self.ewk_Afiq, events)
        ...
        # apply Afiq's weights
        for rewt_type in ["Nominal", "yt_0", "yt_0.5", "yt_1", "yt_1.5", "yt_2"]:
            evt_wt = np.ones(len(events)) if rewt_type == "Nominal" else Afiq_ewk_wts_dict[rewt_type]
        ...
        # apply Otto's weights
        for rewt_type in Otto_ewk_wts_dict.keys():
            evt_wt = Otto_ewk_wts_dict[rewt_type]
