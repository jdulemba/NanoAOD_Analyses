...
class permProbComputer_dev(processor.ProcessorABC):
    def __init__(self):
        ...
        histo_dict = {}
            ## make jet hists
                # all 3j 
        perm_3j_hists = self.make_3j_hists()
        histo_dict.update(perm_3j_hists)
                # 3j merged
    ...
    def make_3j_hists(self):
        histo_dict = {}
        histo_dict['mthad']         = hist.Hist("Events", self.dataset_axis, self.jetmult_axis, self.leptype_axis, self.lepcat_axis, self.mtregion_axis, self.permcat_axis, self.tMass_axis)
        ...
        for lost_evt in np.where(correct_lost)[0]:
            for tp in tps[lost_evt]:
                ...
                accumulator['Lost_mbpjet'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat=tp_cat, mbpjet=np.array([mbpjet]), weight=np.array([evt_weights[lost_evt]]))
                    ...
                    ## fill hists for all 3j events
                accumulator['nusolver_chi2'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Lost' % tp_cat, nu_chi2=np.array([tp_nu[3]]), weight=np.array([evt_weights[lost_evt]]))
                accumulator['nusolver_dist'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Lost' % tp_cat, nu_dist=np.array([np.sqrt(tp_nu[3])]), weight=np.array([evt_weights[lost_evt]]))
                accumulator['mthad'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Lost' % tp_cat, topmass=np.array([mbpjet]), weight=np.array([evt_weights[lost_evt]]))
        ...
        for merged_evt in np.where(correct_merged)[0]:
            for tp in tps[merged_evt]:
                ...
                accumulator['Merged_mbpjet_vs_maxmjet'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat=tp_cat, maxmjet=np.array([maxmjet]), mbpjet=np.array([mbpjet]), weight=np.array([evt_weights[merged_evt]]))
                    ...
                    ## fill hists for all 3j events
                accumulator['nusolver_chi2'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Merged' % tp_cat, nu_chi2=np.array([tp_nu[3]]), weight=np.array([evt_weights[merged_evt]]))
                accumulator['nusolver_dist'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Merged' % tp_cat, nu_dist=np.array([np.sqrt(tp_nu[3])]), weight=np.array([evt_weights[merged_evt]]))
                accumulator['mthad'].fill(dataset=self.sample_name, jmult=jetmult, leptype=leptype, lepcat=lepcat, mtregion=mtregion, permcat='%s_Merged' % tp_cat, topmass=np.array([mbpjet]), weight=np.array([evt_weights[merged_evt]]))
...
output = processor.run_uproot_job(fileset,
    treename='Events',
    processor_instance=permProbComputer_dev(),
