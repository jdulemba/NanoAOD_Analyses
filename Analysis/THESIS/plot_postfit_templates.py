...
rname = f"IMPACTSv35_lj_data_{args.parity}_m{args.mass}_relw{args.width.replace('.', 'p')}.root"
#basedir = "root://cmseos.fnal.gov//store/user/jdulemba/HeavyHiggsFitter/Thesis_constrainedYt_noEtaT_v1"
#rname = f"IMPACTSv35_lj_data_{args.parity}_m{args.mass}_relw{args.width.replace('.', 'p')}_sig1.root"
rfile = uproot.open(os.path.join(basedir, rname))

signal = f"{args.parity}toTT_M{args.mass}_W{args.width.replace('.','p')}"
year_label = cfeatures.year_labels[args.year]

outdir = os.path.join(plot_outdir, jobid, "Limits_htt_btag_sb_regions", os.path.basename(basedir).split(f"{jobid}_")[-1], f"{args.parity}_m{args.mass}_relw{args.width.replace('.', 'p')}", f"postfit", year_label)
...
procs_groups_dict[f"{signal}_Total"] = ["pos_a", "neg_a", "res_a"]
...
#    for jmult in ["3Jets"]:
        ...
        dirname = f"{channel}_{year_label.replace('VFP','')}_postfit"
        #dirname = f"{channel}_{year_label}_postfit"
        ...
        for proc in procs_list:
            ...
            #vals = np.copy(rfile[dirname][proc].values()) if proc == "data" else np.copy(rfile[dirname][f"{channel}_{year_label}_{proc}"].values())
        
            if not histo:
                bin_edges = np.arange(len(vals)+1, dtype=int)
                histo = hist.Hist("Events", hist.Cat("dataset", "dataset"), hist.Bin("x", "x", bin_edges))
        
            histo.fill(**{"dataset" : proc, "x": np.zeros(0), "weight" : np.zeros(0)})
            histo.values()[(proc,)][:] = vals
                # only add variances to data since MC is taken from 'model'
            if proc == "data":
                variances = np.copy(rfile[dirname][proc].variances())
                histo.values(sumw2=True)[(proc,)][1][:] = variances
        ...
        # plot signal if it exists
        if (f"{signal}_Total",) in histo.values().keys():
            #set_trace()
            _, legname = plt_tools.get_styles(f"{signal}_Total", Plotter.hstyles)
            if not np.array_equal(histo.values()[(f"{signal}_Total",)], np.zeros(nbins)):
                factor = 100.
                Plotter.plot_1D(histo.values()[(f"{signal}_Total",)]*factor, bin_edges, xlimits=(0, nbins), ax=ax, label="%s x %i, $\\hat{g}_{%s} = %.3f^{+%.3f}_{%.3f}$" % (legname, factor, args.parity, mu_cen, mu_up, mu_down), color = "#377eb8")
            else:
                Plotter.plot_1D(histo.values()[(f"{signal}_Total",)], bin_edges, xlimits=(0, nbins), ax=ax, label="%s, $\\hat{g}_{%s} = %.3f^{+%.3f}_{%.3f}$" % (legname, args.parity, mu_cen, mu_up, mu_down), color = "#377eb8")
            #Plotter.plot_1D(histo.values()[(f"{signal}_Total",)], bin_edges, xlimits=(0, nbins), ax=ax, label=legname, color = "#377eb8")
        ...
        #set_trace()
        figname = os.path.join(outdir, "_".join([dirname, signal]))
        fig.savefig(figname)
