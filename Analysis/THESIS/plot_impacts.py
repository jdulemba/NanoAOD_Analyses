...
sig_label = "$%s(%s\ GeV, %s\%%)$, $\ell$+jets" % (args.parity, args.mass, args.width)
...
outdir = os.path.join(plot_outdir, jobid, "Limits_htt_btag_sb_regions", os.path.basename(basedir).split(f"{jobid}_")[-1], f"{args.parity}_m{args.mass}_relw{args.width.replace('.', 'p')}", "Impacts")
...
for idx, nplot in enumerate(nplots):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, sharey=True, figsize=(10.0, 12.0))
    ...
    #set_trace()
    ax1.text(0.5, -0.1, "$\\Delta \\hat{g}_{%s}$" % args.parity, color="k", transform=ax1.transAxes, verticalalignment="bottom", horizontalalignment="center")
