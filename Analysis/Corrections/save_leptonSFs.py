...
indir = os.path.join(proj_dir, "inputs", "data", base_jobid, "lepSFs", "leptonSF_noTAGvariation")
#indir = os.path.join(proj_dir, "inputs", "data", base_jobid, "lepSFs")
...
for lep in leptons.keys():
    for year, fname in leptons[lep]["fnames"].items():
        sf_file = convert_histo_root_file(os.path.join(indir, fname))
        ...
        for idx in range(len(eta_binning)-1):
                # reco/ID SFs
            sf_output[year][lep]["Reco_ID"]["Central"][f"eta_bin{idx}"] = dense_lookup(*(sf_file[(leptons[lep]["pt"]["reco_id"][idx], "dense_lookup")][0], sf_file[(leptons[lep]["pt"]["reco_id"][idx], "dense_lookup")][1][0]))
            sf_output[year][lep]["Reco_ID"]["Error"][f"eta_bin{idx}"] = dense_lookup(*(sf_file[("%s_error" % leptons[lep]["pt"]["reco_id"][idx], "dense_lookup")][0], sf_file[("%s_error" % leptons[lep]["pt"]["reco_id"][idx], "dense_lookup")][1][0]))
                # trigger SFs
            sf_output[year][lep]["Trig"]["Central"][f"eta_bin{idx}"] = dense_lookup(*(sf_file[(leptons[lep]["pt"]["trig"][idx], "dense_lookup")][0], sf_file[(leptons[lep]["pt"]["trig"][idx], "dense_lookup")][1][0]))
            sf_output[year][lep]["Trig"]["Error"][f"eta_bin{idx}"] = dense_lookup(*(sf_file[("%s_error" % leptons[lep]["pt"]["trig"][idx], "dense_lookup")][0], sf_file[("%s_error" % leptons[lep]["pt"]["trig"][idx], "dense_lookup")][1][0]))
