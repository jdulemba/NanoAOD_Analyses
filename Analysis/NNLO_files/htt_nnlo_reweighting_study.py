...
names_list = [[dataset for dataset in sorted(set([key[0] for key in hdict['mtt'].values().keys()]))] for hdict in hdicts.values()] # 'mtt' hardcoded because it has all ttJets event cats
...
for plt_type in ['RECO', 'GEN', 'RESO']:
    ...
    for hname in vars_dict.keys():
        ...
        histos = {rewt: hdict[hname][:, :, args.lepton, :, :].integrate('leptype') for rewt, hdict in hdicts.items()} if plt_type == 'RESO' else {rewt: hdict[hname][:, plt_type, :, args.lepton, :, :].integrate('evt_type').integrate('leptype') for rewt, hdict in hdicts.items()}# process, evt_type, jmult, leptype, btag, lepcat
            ...
            for jmult in sorted(set([key[1] for key in (list(histos.values())[0]).values().keys()])):
                for lepcat in sorted(set([key[3] for key in (list(histos.values())[0]).values().keys()])):
                    for btagregion in sorted(set([key[2] for key in (list(histos.values())[0]).values().keys()])):
                        ...
                        allTT_hslices = {rewt : histo[:, jmult, btagregion, lepcat].integrate('jmult').integrate('lepcat').integrate('btag').integrate('process') for rewt, histo in histos.items()} # use SL+DL+Had events
                        ...
                        for rewt, hslice in allTT_hslices.items():
                            ...
                            if (rewt != nominal_jobid) and (nominal_jobid in rewt_style_dict.keys()):
                                ratio_vals, ratio_bins = Plotter.get_ratio_arrays(num_vals=vals, denom_vals=allTT_hslices[nominal_jobid].values()[()], input_bins=bins)
                        ...
                        for ttcat in process_groups.keys():
                            #set_trace()
                            if ttcat not in sorted(set([key[0] for key in histos[nominal_jobid].values().keys()])): continue
                            TTcat_hslices = {rewt : histo[ttcat, jmult, btagregion, lepcat].integrate('jmult').integrate('lepcat').integrate('btag').integrate('process') for rewt, histo in histos.items()}

                            fig, (ax, rax) = plt.subplots(2, 1, gridspec_kw={"height_ratios": (3, 1)}, sharex=True)
                            fig.subplots_adjust(hspace=.07)
    
                            for rewt, hslice in TTcat_hslices.items():
                                vals, bins = hslice.values()[()], hslice.axis(xaxis_name).edges()
                                ax = Plotter.plot_1D(vals, bins, xlimits=x_lims, ax=ax, histtype='step', label=rewt_style_dict[rewt][0], color=rewt_style_dict[rewt][1])
    
                                if (rewt != nominal_jobid) and (nominal_jobid in rewt_style_dict.keys()):
                                    ratio_vals, ratio_bins = Plotter.get_ratio_arrays(num_vals=vals, denom_vals=TTcat_hslices[nominal_jobid].values()[()], input_bins=bins)
                                    rax.step(ratio_bins, ratio_vals, where='post', **{'linestyle' : '-', 'color' : rewt_style_dict[rewt][1]})
