...
def final_MEsig_templates(fnames):
    ...
    coffea_out = os.path.join(outdir, f"final_templates_lj_MEsig_kfactors_{args.year}_{jobid}.coffea" if args.kfactors else f"final_templates_lj_MEsig_{args.year}_{jobid}.coffea")
    save(histo_dict, coffea_out)
    print(f"{coffea_out} written")
...
if __name__ == "__main__":
    ...
    if "MEreweight_sig" in templates_to_run:
            # find signal files
        base_sig_template_name = f"*_templates_lj_MEsig_kfactors_{args.year}_{jobid}.coffea" if args.kfactors else f"*_templates_lj_MEsig_{args.year}_{jobid}.coffea"
        sig_fnames = fnmatch.filter(os.listdir(input_dir), base_sig_template_name)
        sig_fnames = [os.path.join(input_dir, fname) for fname in sig_fnames]
