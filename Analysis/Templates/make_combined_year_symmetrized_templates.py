...
def symmetrize_bkg_templates(fnames_to_run):
        ...
        for sys in systypes:
            ...
            for proc in procs_sys:
                for lep in hdict.keys():
                    ...
                    #set_trace()
                    avg_rel_vals = (up_rel_vals + -1*dw_rel_vals)/2
    ...
    #set_trace()
    if "3Jets" in njets_to_run:
        coffea_out_3j = os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_3Jets_bkg_{jobid}.coffea" if args.treatment == "smooth" else f"test_CombinedSymmetrizedFlattened_templates_lj_3Jets_bkg_{jobid}.coffea")
        save(histo_dict_3j, coffea_out_3j)
        print(f"{coffea_out_3j} written")
    if "4PJets" in njets_to_run:
        coffea_out_4pj = os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_4PJets_bkg_{jobid}.coffea" if args.treatment == "smooth" else f"test_CombinedSymmetrizedFlattened_templates_lj_4PJets_bkg_{jobid}.coffea")
...
def symmetrize_sig_templates(fnames_to_run):
    ...
    if "3Jets" in njets_to_run:
        coffea_out_3j = os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_3Jets_sig_{jobid}.coffea" if args.treatment == "smooth" else f"test_CombinedSymmetrizedFlattened_templates_lj_3Jets_sig_{jobid}.coffea")
        save(histo_dict_3j, coffea_out_3j)
        print(f"{coffea_out_3j} written")
    if "4PJets" in njets_to_run:
        coffea_out_4pj = os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_4PJets_sig_{jobid}.coffea" if args.treatment == "smooth" else f"test_CombinedSymmetrizedFlattened_templates_lj_4PJets_sig_{jobid}.coffea")
        save(histo_dict_4pj, coffea_out_4pj)
        print(f"{coffea_out_4pj} written")
