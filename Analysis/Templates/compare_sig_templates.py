...
parser.add_argument("type", choices=["MC", "Folded", "Folded_LO", "MEreweighting_LO"], help="Choose between signal produced via MC or Folding.")
parser.add_argument("plot", choices=["Comp", "Indiv"], help="Choose between plots with individual variations or comparisons.")
...
if args.type == "MC":
    ...
    output_dir = os.path.join(proj_dir, "plots", f"{args.year}_{jobid}", f"Templates_{analyzer}", "MEreweight_LO_SIG", template_version, args.lepton)
    template_fname = os.path.join(template_dname, f"{template_version}_desy_mew/templates_lj_sig_{args.year}.root")
...
def proc_to_name(proc):
    new_proc = proc.replace("A", "AtoTT_").replace("H", "HtoTT_").replace("relw", "W").replace("int", "Int").replace("res", "Res") # get almost correct format except for mass
...
for jmult in ["3Jets", "4PJets"]:
    ...
    if args.plot == "Comp":
        ...
        for sys in systypes:
            ...
            up_sysname = systematics.combine_template_sys_to_name[args.year][systematics.sys_groups[args.year][sys][0]] if systematics.sys_groups[args.year][sys][0] in systematics.combine_template_sys_to_name[args.year].keys() else None
            dw_sysname = systematics.combine_template_sys_to_name[args.year][systematics.sys_groups[args.year][sys][1]] if systematics.sys_groups[args.year][sys][1] in systematics.combine_template_sys_to_name[args.year].keys() else None
            ...
            if "LEP" in baseSysname:
                baseSysname = baseSysname.replace("LEP", args.lepton[0].lower())
                up_sysname = up_sysname.replace("LEP", args.lepton[0].lower())
                dw_sysname = dw_sysname.replace("LEP", args.lepton[0].lower())
            ...
            for proc in procs_sys:
                if ("A_m500_w10p0" not in proc): continue
                ...
                #set_trace()
                proc_labelname = proc.replace("A", "AtoTT").replace("H", "HtoTT").replace("m", "M").replace("w", "W").replace('neg', 'Int_neg').replace('pos', 'Int_pos').replace("res", "Res")
                ax.legend(loc="upper right", title=f"{sys}, {plt_tools.get_label(proc_labelname, hstyles)}")
    ...
    if args.plot == "Indiv":
        ...
        for sys in systypes:
            if sys == "nosys":
                procs_sys = sorted(set([dist for dist in sig_dists if (dist.endswith("int_neg") or dist.endswith("int_pos") or dist.endswith("res"))]))
                if not procs_sys: continue
                for proc in procs_sys:
                    if ("A500_relw10p0" not in proc): continue
                    #if (proc != "A1000_relw25p0_res"): continue
                    ...
                    #set_trace()
                    pltdir = os.path.join(output_dir, jmult, "Indiv", sys)
                    if not os.path.isdir(pltdir):
                        os.makedirs(pltdir)
                    ...
                    bins = [ax.edges() for ax in rfile[f"{dirname}/{proc}"].axes][0]
                    nosys_vals = np.abs(rfile[f"{dirname}/{proc}"].values())
                    ...
                    ax.step(bins, np.r_[nosys_vals, nosys_vals[-1]], where="post", **{"color" : "k", "linestyle" : "-", "label" : plt_tools.get_label(proc_to_name(proc), hstyles)})
                    ...
                    ax.set_ylabel("Events")
                        ...
                        # add lepton/jet multiplicity label
                    ax.text(
                        0.02, 0.86, f"{leptypes[args.lepton]}, {jet_mults[jmult]}\ndetector level\n{(args.type).replace('_', ' ')}",
                        fontsize=rcParams["font.size"]*0.9, horizontalalignment="left", verticalalignment="bottom", transform=ax.transAxes
                    )
                        ## draw vertical lines for distinguishing different ctstar bins
                    vlines = [x_lims[1]*ybin/5 for ybin in range(1, 5)]
                    for vline in vlines:
                        ax.axvline(vline, ymin=0., ymax=0.9, color="k", linestyle="--")
                        #ax.axvline(vline, color="k", linestyle="--")

                    hep.cms.label(ax=ax, data=False, year=args.year, lumi=round(data_lumi_year[f"{args.lepton}s"]/1000., 1))
                    
                    #set_trace()
                    figname = os.path.join(pltdir, "_".join([jmult, args.lepton, sys, proc]))
                    fig.savefig(figname)
                    print(f"{figname} written")
                    plt.close(fig)
            ...
            #        bins = [ax.edges() for ax in rfile[f"{dirname}/{proc}_{up_sysname}_orig"].axes][0]
            #        nosys_vals = rfile[f"{dirname}/{proc}"].values()
            #        orig_up = rfile[f"{dirname}/{proc}_{up_sysname}_orig"].values()
            #        orig_dw = rfile[f"{dirname}/{proc}_{dw_sysname}_orig"].values()
            #        smooth_up = rfile[f"{dirname}/{proc}_{up_sysname}SMOOTH"].values()
            #        smooth_dw = rfile[f"{dirname}/{proc}_{dw_sysname}SMOOTH"].values()
