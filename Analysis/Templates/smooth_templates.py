...
def smoothing(nominal, template, nbinsx, nbinsy, debug=False):
    ...
    for ybin in range(nbinsy):
        ...
        total_array[ybin*nbinsx:(ybin+1)*nbinsx] = LOWESS(yin, xin, frac=2./3, it=3, return_sorted=False)
    ...
    for bkg_file in bkg_fnames:
        ...
        for lep in hdict.keys():
            ...
            for tname in hdict[lep].keys():
                ...
                #if (sys != 'nosys'): set_trace()

                if (sys != 'nosys') and (templates_to_smooth[proc]):
                    template_histo = smoothing(nominal=histo_dict_3j[lep]['%s_nosys' % proc] if jmult == '3Jets' else histo_dict_4pj[lep]['%s_nosys' % proc], template=template_histo, nbinsx=len(linearize_binning[0])-1, nbinsy=len(linearize_binning[1])-1)
                    #set_trace()
                
                    ## save template histos to coffea dict
                if jmult == '3Jets':
                    histo_dict_3j[lep][tname] = template_histo
                if jmult == '4PJets':
                    histo_dict_4pj[lep][tname] = template_histo

    #set_trace()
    if '3Jets' in njets_to_run:
        coffea_out_3j = '%s/templates_lj_3Jets_bkg_smoothed_%s_%s.coffea' % (outdir, jobid, args.year)
        save(histo_dict_3j, coffea_out_3j)
        print("%s written" % coffea_out_3j)
    if '4PJets' in njets_to_run:
        coffea_out_4pj = '%s/templates_lj_4PJets_bkg_smoothed_%s_%s.coffea' % (outdir, jobid, args.year)
