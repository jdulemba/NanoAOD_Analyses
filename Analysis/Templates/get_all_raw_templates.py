...
def get_bkg_templates(tmp_rname):
    ...
    if '3Jets' in njets_to_run:
        coffea_out_3j = '%s/raw_templates_lj_3Jets_bkg_%s_%s.coffea' % (outdir, jobid, args.year)
        save(histo_dict_3j, coffea_out_3j)
        print("%s written" % coffea_out_3j)
    if '4PJets' in njets_to_run:
        coffea_out_4pj = '%s/raw_templates_lj_4PJets_bkg_%s_%s.coffea' % (outdir, jobid, args.year)
...
def write_correct_template_format(in_fname, isSignal=None):
    ...
    if '3Jets' in njets_to_run:    
        ...
        fname_3j = '%s/raw_templates_lj_3Jets_%s_%s_%s.root' % (outdir, signame, jobid, args.year)
    ...
    if '4PJets' in njets_to_run:
        ...
        fname_4pj = '%s/raw_templates_lj_4PJets_%s_%s_%s.root' % (outdir, signame, jobid, args.year)
...
if __name__ == '__main__':
    ...
    outdir = os.path.join(proj_dir, 'Templates', 'results', jobid, args.year)
