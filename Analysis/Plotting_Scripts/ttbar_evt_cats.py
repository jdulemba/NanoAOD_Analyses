...
parser.add_argument('year', choices=['2016', '2018'], help='What year is the ntuple from.')
...
general_cat_styles = {
    'Lost' : {'facecolor' : '#377eb8', 'name' : 'Lost'},
    'Merged' : {'facecolor' : '#e41a1c', 'name' : 'Partially Merged'},
    'Other' : {'facecolor' : '#808080', 'name' : 'Other'},
    'Sum unc.' : {
        'facecolor' : 'none',
        'linewidth' : 0,
...
for hname in variables.keys():
    ...
    h_mu.scale(lumi_correction[args.year]['Muons'], axis='dataset')
    h_el = histo[:, :, 'Electron', :, :].integrate('leptype')
    h_el.scale(lumi_correction[args.year]['Electrons'], axis='dataset')
    h_tot = h_mu+h_el
    ...
    if h_tot.dense_dim() == 1:
        ...
        for jmult in list(set([key[0] for key in h_tot.values().keys()])):
            for mtreg in list(set([key[1] for key in h_tot.values().keys()]))+['All_MT']:
                pltdir = os.path.join(outdir, jmult, mtreg)
                if not os.path.isdir(pltdir):
                    os.makedirs(pltdir)
                print(hname, jmult, mtreg)    

                hslice = h_tot[jmult, :].integrate('jmult').integrate('mtreg') if mtreg == 'All_MT' else h_tot[jmult, mtreg].integrate('jmult').integrate('mtreg')
                if rebinning != 1:
                ...
                ax = Plotter.plot_mc1d(ax=ax, hdict=indiv_cat_histo, xlabel=xtitle, ylabel='Events', xlimits=x_lims, hist_styles=indiv_cat_styles, **{'error_opts':None})
                    # add lep category
                ax.text(
                    0.02, 0.90, "$e/\mu$, %s          %s\nParton Level" % (jet_mults[jmult], mt_dict[mtreg]),
                ...
                #set_trace()
                figname = os.path.join(pltdir, '_'.join([jmult, mtreg, 'Indiv_Cats', hname]))
                fig.savefig(figname)
                ...
                ax_norm = Plotter.plot_mc1d(ax=ax_norm, hdict=indiv_norm_histo, xlabel=xtitle, ylabel='Fraction', xlimits=x_lims, ylimits=(0., 1.), hist_styles=indiv_cat_styles, **{'error_opts':None})
                    # add lep category
                ax_norm.text(
                    0.02, 0.90, "$e/\mu$, %s          %s\nParton Level" % (jet_mults[jmult], mt_dict[mtreg]),
                    ...
                    ## set axes labels and titles
                hep.cms.cmslabel(ax=ax_norm, data=False, paper=False, year=args.year, lumi=round(lumi_to_use, 1))

                #set_trace()
                figname_norm = os.path.join(pltdir, '_'.join([jmult, mtreg, 'Indiv_Cats', hname, 'Norm']))
                ...
                ax = Plotter.plot_mc1d(ax=ax, hdict=general_cat_histo, xlabel=xtitle, ylabel='Events', xlimits=x_lims, hist_styles=general_cat_styles, **{'error_opts':None})
                    # add lep category
                ax.text(
                    0.02, 0.90, "$e/\mu$, %s          %s\nParton Level" % (jet_mults[jmult], mt_dict[mtreg]),
                    fontsize=rcParams['font.size'], horizontalalignment='left', verticalalignment='bottom', transform=ax.transAxes
                ...
                #set_trace()
                figname = os.path.join(pltdir, '_'.join([jmult, mtreg, 'Group_Cats', hname]))
                fig.savefig(figname)
                print('%s written' % figname)
                ...
                ax_norm = Plotter.plot_mc1d(ax=ax_norm, hdict=gen_norm_histo, xlabel=xtitle, ylabel='Fraction', xlimits=x_lims, ylimits=(0., 1.), hist_styles=general_cat_styles, **{'error_opts':None})
                    # add lep category
                ax_norm.text(
                    0.02, 0.90, "$e/\mu$, %s          %s\nParton Level" % (jet_mults[jmult], mt_dict[mtreg]),
                    fontsize=rcParams['font.size'], horizontalalignment='left', verticalalignment='bottom', transform=ax_norm.transAxes
                ...
                #set_trace()
                figname_norm = os.path.join(pltdir, '_'.join([jmult, mtreg, 'Group_Cats', hname, 'Norm']))
                fig_norm.savefig(figname_norm)
                print('%s written' % figname_norm)
                plt.close()
                        ...
                        ## make table
                rows = [("Lumi: %s fb^-1" % format(round(lumi_to_use, 1), '.1f'), 'e/mu', jmult, mtreg)]
                ...
                cat_fractions_name = os.path.join(pltdir, 'Event_Categories_%s_yields_and_fractions.txt' % '_'.join([jmult, mtreg]))
