...
indiv_cat_styles = {
    ...
    "Lost_BLep" : {"facecolor" : "#e31a1c", "name" : "Lost $b_{\ell}$"},
    ...
    "Merged_BLepWJa" : {"facecolor" : "#33a02c", "name" : "Merged $b_{\ell}$, $W_{ja}$"},
    "Merged_BLepWJb" : {"facecolor" : "#cab2d6", "name" : "Merged $b_{\ell}$, $W_{jb}$"},
...
variables = {
    "mtt" : (cfeatures.variable_names_to_labels["mtt"], 4, (300., 2000.)),
...
for hname in variables.keys():
    if not hname in hdict.keys():
        raise ValueError(f"{hname} not found")
    ...
    if h_tot.dense_dim() == 1:
        ...
        for jmult in sorted(set([key[0] for key in h_tot.values().keys()])):
            ...
            #set_trace()
            cat_fractions_name = os.path.join(pltdir, f"{args.year}_{jobid}_Event_Categories_{jmult}_yields_and_fractions.txt")
            plt_tools.print_table(rows, filename=cat_fractions_name, header_line=1, print_output=True)
            print(f"{cat_fractions_name} written")
