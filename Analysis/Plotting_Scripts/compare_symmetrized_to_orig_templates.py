...
outdir = os.path.join(plot_outdir, jobid if args.combination == "era_lepton" else f"{args.year}_{jobid}", f"Compare_Symmetrized_to_Orig_Templates_{analyzer}", (args.process).upper(), (args.combination).upper())
...
if args.process == "bkg":
    if args.combination == "lepton":
        ...
        smoothed_hdict = load(os.path.join(eos_dir, "results", f"{args.year}_{jobid}", f"Templates_{analyzer}", f"symmetrized_smoothed_combined_lep_templates_lj_bkg_{args.year}_{jobid}.coffea"))
        flattened_hdict = load(os.path.join(eos_dir, "results", f"{args.year}_{jobid}", f"Templates_{analyzer}", f"symmetrized_flattened_combined_lep_templates_lj_bkg_{args.year}_{jobid}.coffea"))
        ...
        "JES_FlavorQCD", "JES_RelativeBal", f"JES_RelativeSample_{args.year}", "MET"]
    ...
    if args.combination == "era_lepton":
        ...
        smoothed_hdict = load(os.path.join(eos_dir, "results", jobid, f"Templates_{analyzer}", f"symmetrized_smoothed_combined_year_and_lepton_templates_lj_bkg_{jobid}.coffea"))
        flattened_hdict = load(os.path.join(eos_dir, "results", jobid, f"Templates_{analyzer}", f"symmetrized_flattened_combined_year_and_lepton_templates_lj_bkg_{jobid}.coffea"))
    ...
    if args.combination == "indiv":
        ...
        smoothed_hdict = load(os.path.join(eos_dir, "results", f"{args.year}_{jobid}", f"Templates_{analyzer}", f"symmetrized_smoothed_templates_lj_bkg_{args.year}_{jobid}.coffea"))
        flattened_hdict = load(os.path.join(eos_dir, "results", f"{args.year}_{jobid}", f"Templates_{analyzer}", f"symmetrized_flattened_templates_lj_bkg_{args.year}_{jobid}.coffea"))
...
if args.combination == "indiv":
    for jmult in sorted(orig_hdict.keys()):
        for lep in ["Muon", "Electron"]:
            ...
            flat_dict = flattened_hdict[jmult][lep]  
            ...
            for sys in systypes:
                ...
                for proc in procs_sys:
                    ...
                    flat_exists = (f"{proc}_{up_sysname}" in flat_dict.keys()) & (f"{proc}_{dw_sysname}" in flat_dict.keys())
                    ...
                    up_histos.append((orig_nominal, orig_up, {"color": "r", "linestyle": "-", "label": f"Original Up"}, True))
                    dw_histos.append((orig_nominal, orig_dw, {"color": "b", "linestyle": "-", "label": f"Original Down"}, True))
    
                    if smooth_exists:
                        smooth_up, smooth_dw = smooth_dict[f"{proc}_{up_sysname}"].copy(), smooth_dict[f"{proc}_{dw_sysname}"].copy()
                        up_histos.append((orig_nominal, smooth_up, {"color": "g", "linestyle": "-", "label": f"Smooth Up"}, False))
                        dw_histos.append((orig_nominal, smooth_dw, {"color": "k", "linestyle": "-", "label": f"Smooth Down"}, False))
    
                    if flat_exists:
                        flat_up, flat_dw = flat_dict[f"{proc}_{up_sysname}"].copy(), flat_dict[f"{proc}_{dw_sysname}"].copy()
                        up_histos.append((orig_nominal, flat_up, {"color": "r", "linestyle": "-", "label": f"Flat Up"}, False))
                        dw_histos.append((orig_nominal, flat_dw, {"color": "b", "linestyle": "-", "label": f"Flat Down"}, False))
                    ...
                    #set_trace()
                    figname = os.path.join(pltdir, "_".join([proc, jmult, lep, combine_sysname, "Indiv_Symm_to_Orig_Comp"]))
                    fig.savefig(figname)
    ...
    for jmult in sorted(orig_hdict.keys()):
        ...
        for sys in systypes:
            ...
            for proc in procs_sys:
                ...
                flat_exists = (f"{proc}_{up_sysname}" in flattened_hdict[jmult].keys()) & (f"{proc}_{dw_sysname}" in flattened_hdict[jmult].keys())
                if not (smooth_exists and flat_exists): continue
                #if not smooth_exists: continue
                ...
                orig_nominal, orig_up, orig_dw = orig_dict[f"{proc}_nosys"].copy(), orig_dict[f"{proc}_{up_sysname}"].copy(), orig_dict[f"{proc}_{dw_sysname}"].copy()
                up_histos.append((orig_nominal, orig_up, {"color": "r", "linestyle": "-", "label": "Original Up"}, True))
                dw_histos.append((orig_nominal, orig_dw, {"color": "b", "linestyle": "-", "label": "Original Down"}, True))
    
                if smooth_exists:
                    smooth_up, smooth_dw = smoothed_hdict[jmult][f"{proc}_{up_sysname}"].copy(), smoothed_hdict[jmult][f"{proc}_{dw_sysname}"].copy()
                    up_histos.append((None, smooth_up, {"color": "g", "linestyle": "-", "label": "Smooth Up"}, False))
                    dw_histos.append((None, smooth_dw, {"color": "k", "linestyle": "-", "label": "Smooth Down"}, False))
    
                if flat_exists:
                    flat_up, flat_dw = flattened_hdict[jmult][f"{proc}_{up_sysname}"].copy(), flattened_hdict[jmult][f"{proc}_{dw_sysname}"].copy()
                    up_histos.append((None, flat_up, {"color": "r", "linestyle": "-", "label": "Flat Up"}, False))
                    dw_histos.append((None, flat_dw, {"color": "b", "linestyle": "-", "label": "Flat Down"}, False))
                ...
                #set_trace()
                figname = os.path.join(pltdir, "_".join([proc, jmult, combine_sysname, "Symm_to_Orig_Comp"]))
                fig.savefig(figname)
                print(f"{figname} written")
                plt.close(fig)
