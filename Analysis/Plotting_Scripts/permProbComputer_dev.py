...
perm_cats = {
    '3Jets' : {
        #'Correct' : ['Correct'],
        #'Wrong' : ['Wrong'],
        'Correct' : ['Correct_Lost', 'Correct_Merged'],
        'Wrong' : ['Wrong_Lost', 'Wrong_Merged'],
    },
    '4PJets' : {
        '1D' : {
...
variables_3jets = {
    #'nusolver_chi2' : ('$\\chi_{\\nu}^{2}$', 5, (0., 1000.), False),
    #'nusolver_dist' : ('$D_{\\nu, min}$ [GeV]', 1, (0., 150.), False),
    'mthad' : ('m($t_{h}$) [GeV]', 1, (0., 500.), False),
            ...
            if h_tot.dense_dim() == 1:
                ...
                for jmult in list(set([key[0] for key in h_tot.values().keys()])):
                    for lepcat in list(set([key[1] for key in h_tot.values().keys()])):
                        pltdir = os.path.join(outdir, jmult, lepcat)
                        ...
                        #ax.legend(handles,labels, loc='upper right')
                            ...
                            # add lep category
                        ax.text(
                            0.02, 0.90, "$e/\mu$, %s" % jet_mults[jmult],
