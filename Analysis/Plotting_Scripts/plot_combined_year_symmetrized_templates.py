...
outdir = os.path.join(proj_dir, "plots", jobid, f"Templates_{analyzer}", "Symmetrized", args.lepton)
...
templates_names = {
    "3Jets" : {
        "bkg" : {
            "Orig" : load(os.path.join(input_dir, f"test_CombinedRaw_templates_lj_3Jets_bkg_{jobid}.coffea")),
            "Smooth" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_3Jets_bkg_{jobid}.coffea")),
            "Flat" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedFlattened_templates_lj_3Jets_bkg_{jobid}.coffea")),
            },
        "sig" : {
            "Orig" : load(os.path.join(input_dir, f"test_CombinedRaw_templates_lj_3Jets_sig_{jobid}.coffea")),
            "Smooth" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_3Jets_sig_{jobid}.coffea")),
            "Flat" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedFlattened_templates_lj_3Jets_sig_{jobid}.coffea")),
            },
    },
    "4PJets" : {
        "bkg" : {
            "Orig" : load(os.path.join(input_dir, f"test_CombinedRaw_templates_lj_4PJets_bkg_{jobid}.coffea")),
            "Smooth" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_4PJets_bkg_{jobid}.coffea")),
            "Flat" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedFlattened_templates_lj_4PJets_bkg_{jobid}.coffea")),
            },
        "sig" : {
            "Orig" : load(os.path.join(input_dir, f"test_CombinedRaw_templates_lj_4PJets_sig_{jobid}.coffea")),
            "Smooth" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedSmoothed_templates_lj_4PJets_sig_{jobid}.coffea")),
            "Flat" : load(os.path.join(input_dir, f"test_CombinedSymmetrizedFlattened_templates_lj_4PJets_sig_{jobid}.coffea")),
            },
    },
}

data_lumi_year = prettyjson.loads(open(os.path.join(proj_dir, "inputs", f"{base_jobid}_lumis_data.json")).read())["TOT"]

nom_styles = {"color":"k", "linestyle":"-", "label":"Nominal"}
orig_styles = {"color":"b", "linestyle":"-", "label":"Original"}
smooth_styles = {"color":"r", "linestyle":"-", "label":"Smoothed, Symm"}
flat_styles = {"color":"g", "linestyle":"-", "label":"Flattened, Symm"}
    ...
    if args.indiv:
        for sys in systs:
            ...
            for proc_sys in procs_sys:
                ...
                hep.cms.label(ax=ax, data=False, lumi=round(data_lumi_year["%ss" % args.lepton]/1000., 1))
    ...
    if args.comp:
        ...
        for sys in systypes:
            ...
            for proc in procs_sys:
                ...
                #if (sys == "AH_FACTOR") and (proc == "AtoTTJetsSL_M1000_W2p5_Int_neg"): set_trace()
