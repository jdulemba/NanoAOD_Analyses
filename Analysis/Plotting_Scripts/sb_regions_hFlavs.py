...
btag_reg_names_dict = {
    'Signal' : {'reg' : 'btagPass', 'leg' : '$n_{btags} \geq$ 2'},
    'Central': {'reg' : 'p15p30', 'leg' : '0.15 < max(btag discr) $\leq$ 0.30', 'label' : 'Cen (0.15-0.3)', 'color' : 'k'},
    'Up'     : {'reg' : 'p30p45', 'leg' : '0.30 < max(btag discr) $\leq$ 0.45', 'label' : 'Up (0.3-0.45)', 'color' : 'r'},
    'Down'   : {'reg' : 'p00p15', 'leg' : '0.00 < max(btag discr) $\leq$ 0.15', 'label' : 'Down (0.0-0.15)', 'color' : 'b'}
}

variables = {
    'DeepCSV_bDisc' : ('DeepCSV b Disc', 1, (-0.01, 1.), True),
    'DeepCSV_bDisc_vs_Jet_pt' : ('$p_{T}$(jets)', 'DeepCSV b Disc', 1, 1, (0., 300.), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet_eta' : ('$\\eta$(jets)', 'DeepCSV b Disc', 1, 1, (-2.6, 2.6), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet1_pt' : ('$p_{T}$(jet 1)', 'DeepCSV b Disc', 1, 1, (0., 300.), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet1_eta' : ('$\\eta$(jet 1)', 'DeepCSV b Disc', 1, 1, (-2.6, 2.6), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet2_pt' : ('$p_{T}$(jet 2)', 'DeepCSV b Disc', 1, 1, (0., 300.), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet2_eta' : ('$\\eta$(jet 2)', 'DeepCSV b Disc', 1, 1, (-2.6, 2.6), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet3_pt' : ('$p_{T}$(jet 3)', 'DeepCSV b Disc', 1, 1, (0., 300.), (-0.01, 1.), True, None, (None, None)),
    'DeepCSV_bDisc_vs_Jet3_eta' : ('$\\eta$(jet 3)', 'DeepCSV b Disc', 1, 1, (-2.6, 2.6), (-0.01, 1.), True, None, (None, None)),
}
        ...
        if histo.dense_dim() == 1:
            ...
            for jmult in sorted(set([key[2] for key in histo.values().keys()])):
                ...
                for btagregion in sorted(set([key[1] for key in histo.values().keys()])):
                    ...
                    # make plots integrating quark flavor
                    fig, (ax, rax) = plt.subplots(2, 1, gridspec_kw={"height_ratios": (3, 1)}, sharex=True)
                    fig.subplots_adjust(hspace=.07)
    
                    htot = histo[:, btagregion, jmult, :].integrate('jmult').integrate('btag').integrate('hFlav')
                    ...
                    #set_trace() 
                    Plotter.plot_stack1d(ax, rax, htot, xlabel=xtitle, xlimits=x_lims, **mc_opts)
                    ...
                    plt.close()
                        ...
                        # make plots for each quark flavor (MC only)
                    for hflav in sorted(set([key[3] for key in histo.values().keys()])):   
                        fig, ax = plt.subplots()
                        fig.subplots_adjust(hspace=.07)
    
                        #set_trace()
                        flav_histo = histo[:, btagregion, jmult, hflav].integrate('jmult').integrate('btag').integrate('hFlav')
                        hslice = flav_histo[Plotter.mc_samples]
                        ...
                        #set_trace() 
                        Plotter.plot_mc1d(ax, hslice, xlabel='%s, %s quarks' % (xtitle, flav_to_name[hflav]), xlimits=x_lims, **mc_opts)
                        ...
                        #set_trace()
                        figname = os.path.join(pltdir, '_'.join([jmult, args.lepton, btagregion, hname, hflav]))
                        fig.savefig(figname)
                        print('%s written' % figname)
                        plt.close()
        ...
        if histo.dense_dim() == 2:
            ...
            for jmult in sorted(set([key[2] for key in histo.values().keys()])):
                ...
                for btagregion in sorted(set([key[1] for key in histo.values().keys()])):
                    ...
                    #set_trace() 
                    # make plots integrating quark flavor
                    underflow_hist = hslice[:, :, :, :0.00].integrate('hFlav').integrate('deepcsv_bdisc', overflow='under')
                    normal_reg_hist = hslice.integrate('hFlav').integrate('deepcsv_bdisc')
                    ...
                    for hflav in sorted(set([key[3] for key in histo.values().keys()])):   
                        #set_trace()
                        underflow_flav_hist = hslice[:, hflav, :, :0.00].integrate('hFlav').integrate('deepcsv_bdisc', overflow='under')
                        normal_reg_flav_hist = hslice[:, hflav].integrate('hFlav').integrate('deepcsv_bdisc')
                        underflow_hist = underflow_flav_hist[Plotter.mc_samples]
                        normal_reg_hist = normal_reg_flav_hist[Plotter.mc_samples]

                            # plot 1D projection of underflow bin
                        fig_und, ax_und = plt.subplots()
                        fig_und.subplots_adjust(hspace=.07)

                        Plotter.plot_mc1d(ax_und, underflow_hist, xlabel='%s DeepCSV underflow, %s quarks' % (xtitle, flav_to_name[hflav]), xlimits=x_lims, **mc_opts)
                        ...
                        hep.cms.label(ax=ax_und, data=False, paper=False, year=args.year, lumi=round(data_lumi_year['%ss' % args.lepton]/1000., 1))
    
                        #set_trace()
                        und_figname = os.path.join(pltdir, '_'.join([jmult, args.lepton, btagregion, hname, hflav, 'DeepCSV_underflow', 'proj']))
                        fig_und.savefig(und_figname)
                        print('%s written' % und_figname)
                        plt.close()
                        ...
                        Plotter.plot_mc1d(ax_norm, normal_reg_hist, xlabel='%s DeepCSV normal region, %s quarks' % (xtitle, flav_to_name[hflav]), xlimits=x_lims, **mc_opts)
                        ...
                        hep.cms.label(ax=ax_norm, data=False, paper=False, year=args.year, lumi=round(data_lumi_year['%ss' % args.lepton]/1000., 1))
    
                        #set_trace()
                        norm_figname = os.path.join(pltdir, '_'.join([jmult, args.lepton, btagregion, hname, hflav, 'DeepCSV_normal', 'proj']))
                        fig_norm.savefig(norm_figname)
                        print('%s written' % norm_figname)
                        plt.close()
